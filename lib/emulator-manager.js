 ...
 */
function launchEmulator(apiLevel, target, arch, profile, cores, ramSize, heapSize, sdcardPathOrSize, diskSize, avdName, forceAvdCreation, emulatorOptions, disableAnimations, disableSpellChecker, disableLinuxHardwareAcceleration, enableHardwareKeyboard, afterBootDelay, hwuiRenderer) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            yield waitForDevice();
            if (hwuiRenderer) {
                yield printUiRendererValue();
                console.log(`Setting renderer to ${hwuiRenderer}.`);
                yield exec.exec(`adb shell "su root setprop debug.hwui.renderer ${hwuiRenderer}"`);
                yield exec.exec(`adb shell "su root stop"`);
                yield exec.exec(`adb shell "su root start"`);
                ...
                yield delay(hwuiRendererWait * 1000);
                yield printUiRendererValue();
            }
            if (afterBootDelay) {
...
function printUiRendererValue() {
    return __awaiter(this, void 0, void 0, function* () {
        // Display property value
        try {
            let result = '';
            yield exec.exec(`adb shell getprop debug.hwui.renderer`, [], {
                listeners: {
                    stdout: (data) => {
                        result += data.toString();
                    },
